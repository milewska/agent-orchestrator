...
describe("getLaunchCommand", () => {
  ...
  it("includes prompt as positional argument (not -p flag)", () => {
    const cmd = agent.getLaunchCommand(makeLaunchConfig({ prompt: "Fix the bug" }));
    expect(cmd).not.toContain("-p");
    expect(cmd).toContain("'Fix the bug'");
  });

  it("combines all options with prompt as positional arg", () => {
    const cmd = agent.getLaunchCommand(
  ...
  it("includes --append-system-prompt and prompt as positional arg", () => {
    const cmd = agent.getLaunchCommand(
  ...
  it("uses systemPromptFile via shell substitution with prompt as positional arg", () => {
    const cmd = agent.getLaunchCommand(
